...
CJusin::CJusin(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CJusin::Ready_Object(void)
{
	...
	m_pTransCom->m_vScale.x = 140.f;
	m_pTransCom->m_vScale.y = 140.f;
...
_int CJusin::Update_Object(const _float& fTimeDelta)
{
	...
	{
		if (1.75f <= dynamic_cast<CDynamicCamera*>(Engine::Find_DynamicCameraObject())->Get_At().y)
		{
			if (255.f > m_fAlpha)
				m_fAlpha += 170.f * fTimeDelta;
			else
...
_int CJusin::LateUpdate_Object(const _float& fTimeDelta)
{
...
void CJusin::PreRender_Object(void)
{
...
void CJusin::Render_Object(void)
{
...
void CJusin::PostRender_Object(void)
{
...
HRESULT CJusin::Add_Component(void)
{
	...
	/*  Texture  */
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_LOGO, L"Texture_Jusin"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CJusin * CJusin::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CJusin *	pInstance = new CJusin(pGraphicDev);
