...
CCup::CCup(LPDIRECT3DDEVICE9 _pGraphicDev)
...
HRESULT CCup::Ready_Object(void)
{
...
_int CCup::Update_Object(const _float& fTimeDelta)
{
	if (FALSE == m_bDrink)
		return UPDATE_OK;
...
_int CCup::LateUpdate_Object(const _float& fTimeDelta)
{
...
void CCup::PreRender_Object(void)
{
	m_pGraphicDev->SetTransform(D3DTS_WORLD, &m_pTransCom->m_matWorld);
}

void CCup::Render_Object(void)
{
	PreRender_Object();
...
void CCup::PostRender_Object(void)
{

}

_int CCup::CollisionUpdate_Object(const _float& fTimeDelta, 
										const DWORD _dwFlag, 
...
HRESULT CCup::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CCup::Add_Component()
{
	Engine::CComponent*		pComponent = nullptr;
	
	/*  Mesh  */
	pComponent = m_pMeshCom = dynamic_cast<Engine::CStaticMesh*>(Engine::Clone(RESOURCE_STAGE, L"Mesh_Cup_00"));
...
CCup* CCup::Create(LPDIRECT3DDEVICE9 _pGraphicDev, CElite* _pElite)
{
	CCup*	pInstance = new CCup(_pGraphicDev);
