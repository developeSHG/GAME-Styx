...
CBroom::CBroom(LPDIRECT3DDEVICE9 _pGraphicDev)
	: CGameObject(_pGraphicDev)
{
	m_eOwnerObjectID = Engine::OBJECTID::OBJECT_ELITE;
	m_eColObjectID = Engine::COL_OBJECTID::COL_OBJECT_END;
}
...
HRESULT CBroom::Ready_Object(void)
{
...
_int CBroom::Update_Object(const _float& fTimeDelta)
{
...
_int CBroom::LateUpdate_Object(const _float& fTimeDelta)
{
...
void CBroom::PreRender_Object(void)
{
	m_pGraphicDev->SetTransform(D3DTS_WORLD, &m_pTransCom->m_matWorld);
}

void CBroom::Render_Object(void)
{
	PreRender_Object();
...
void CBroom::PostRender_Object(void)
{

}

_int CBroom::CollisionUpdate_Object(const _float& fTimeDelta, 
										const DWORD _dwFlag, 
...
HRESULT CBroom::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CBroom::Set_Born(const BORN& _eBorn)
{
	if (_eBorn != m_eBorn)
	{
		m_eBorn = _eBorn;
...
HRESULT CBroom::Add_Component()
{
	Engine::CComponent*		pComponent = nullptr;
	
	/*  Mesh  */
	pComponent = m_pMeshCom = dynamic_cast<Engine::CStaticMesh*>(Engine::Clone(RESOURCE_STAGE, L"Mesh_Broom"));
...
CBroom* CBroom::Create(LPDIRECT3DDEVICE9 _pGraphicDev, CElite* _pElite)
{
	CBroom*	pInstance = new CBroom(_pGraphicDev);
