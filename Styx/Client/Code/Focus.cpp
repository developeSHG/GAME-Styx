...
CFocus::CFocus(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CFocus::Ready_Object(void)
{
	FAILED_CHECK_RETURN(Engine::CGameObject::Ready_Object(), E_FAIL);

	FAILED_CHECK_RETURN(Add_Component(), E_FAIL);

	m_pTransCom->m_vInfo[Engine::INFO::INFO_POS].x = -650.f;
	m_pTransCom->m_vInfo[Engine::INFO::INFO_POS].y = -460.f;
	m_pTransCom->m_vInfo[Engine::INFO::INFO_POS].z = 1.f;
	m_pTransCom->m_vScale.x = 220.f;
	m_pTransCom->m_vScale.y = 65.f;
...
_int CFocus::Update_Object(const _float& fTimeDelta)
{
	...
	{
		if (1.35f <= dynamic_cast<CDynamicCamera*>(Engine::Find_DynamicCameraObject())->Get_At().y)
		{
			if (255.f > m_fAlpha)
				m_fAlpha += 170.f * fTimeDelta;
			else
...
_int CFocus::LateUpdate_Object(const _float& fTimeDelta)
{
...
void CFocus::PreRender_Object(void)
{
...
void CFocus::Render_Object(void)
{
...
void CFocus::PostRender_Object(void)
{
...
HRESULT CFocus::Add_Component(void)
{
	...
	/*  Texture  */
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_LOGO, L"Texture_Focus"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CFocus * CFocus::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CFocus *	pInstance = new CFocus(pGraphicDev);
