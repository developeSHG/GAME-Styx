...
HRESULT CTerrain::Ready_Object(void)
{
	...
	m_pTransCom->Set_Pos(-6.f, 0.f, TERRAIN_NUM_Z / 1.9f);
	m_pTransCom->Rotation(Engine::ROTATION::ROT_Y, D3DXToRadian(45.f));

	/*  Rc Buffer  */
	//m_pTransCom->Set_Pos(-3.f, 0.f, TERRAIN_NUM_Z / 1.9f);
	//m_pTransCom->Set_Scale(95.f, 0.f, 90.f);
	//m_pTransCom->Rotation(Engine::ROTATION::ROT_Y, D3DXToRadian(45.f));

	//m_pTransCom->Set_Pos(TERRAIN_NUM_X >> 1, 0.f, TERRAIN_NUM_Z / 1.9f);
	//m_pTransCom->Set_Scale(129.f, 0.f, 129.f);
...
void CTerrain::Render_Object(void)
{
	/*  NaviMesh, Collider Render  */
	{
		...
		if (TRUE == Engine::Get_NaviMgr()->m_bCollRender)
