...
CPs4::CPs4(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CPs4::Ready_Object(void)
{
	FAILED_CHECK_RETURN(Engine::CGameObject::Ready_Object(), E_FAIL);

	FAILED_CHECK_RETURN(Add_Component(), E_FAIL);

	m_pTransCom->m_vInfo[Engine::INFO::INFO_POS].x = 520.f;
...
_int CPs4::Update_Object(const _float& fTimeDelta)
{
	...
	{
		if (1.65f <= dynamic_cast<CDynamicCamera*>(Engine::Find_DynamicCameraObject())->Get_At().y)
		{
			if (255.f > m_fAlpha)
				m_fAlpha += 170.f * fTimeDelta;
			else
...
_int CPs4::LateUpdate_Object(const _float& fTimeDelta)
{
...
void CPs4::PreRender_Object(void)
{
...
void CPs4::PostRender_Object(void)
{
...
HRESULT CPs4::Add_Component(void)
{
	...
	/*  Texture  */
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_LOGO, L"Texture_Ps4"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
...
CPs4 * CPs4::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CPs4 *	pInstance = new CPs4(pGraphicDev);
