...
HWND g_hWnd;
D3DFILLMODE g_eFillMode = D3DFILL_SOLID;		// 렌더스테이트(채우기 방식)
BOOL g_bSlowActive = FALSE;
BOOL			g_bLightPointActive = false;
                     ...
                     _In_ int       nCmdShow)
{
	...
	FAILED_CHECK_RETURN(Engine::Ready_Frame(L"Frame60", 10000.f), E_FAIL);
    ...
    while (true)
    {
		...
		else
		{
			...
			if (Engine::IsPermit_Call(L"Frame60", fTime))
			{
				...
				if (FALSE == g_bSlowActive)
				{
					pMainApp->Update_MainApp(fTime60);
					pMainApp->LateUpdate_MainApp(fTime60);
				}
				else if (TRUE == g_bSlowActive)
				{
					fTime60 /= 7.f;
					pMainApp->Update_MainApp(fTime60);
					pMainApp->LateUpdate_MainApp(fTime60);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(szWindowClass, L"Styx Client", WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0,
	   rc.right - rc.left, 
