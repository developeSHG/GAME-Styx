...
int CToolApp::ExitInstance()
{
	...
	/*  ToolMgr Destroy  */
	Engine::Safe_Single_Destory(m_pToolMgr);
...
BOOL CToolApp::OnIdle(LONG lCount)
{
	...
	_float	fTime = Engine::Get_TimeDelta(L"Timer_Immediate");
	...
	if (Engine::IsPermit_Call(L"Frame60", fTime))
	{
		...
		CToolMgr::GetInstance()->Update_ToolMgr(fTime60);
		CToolMgr::GetInstance()->Render_ToolMgr();
